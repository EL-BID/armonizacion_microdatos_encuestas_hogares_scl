...
ds, has(type string)
    foreach var of varlist `r(varlist)' {
		...
		replace `var' = "." if strpos(`var',"ą")
		replace `var' = "." if strpos(`var',"±¸±˛ą°")
		replace `var' = "." if strpos(`var',"˛ą°±ą±")
		replace `var' = "." if strpos(`var',"ąą±±ą°")
		replace `var' = "." if strpos(`var',"ąą±ąą°")
		replace `var' = "." if strpos(`var',"ąąąąąą")
		replace `var' = "." if strpos(`var',".")
		replace `var' = "." if strpos(`var',"999999")
		replace `var' = "." if strpos(`var',"9999")
		replace `var' = "." if strpos(`var',"999")
		       destring `var', replace
    }



** fecha de entrevista (int_date):
/*
...
gen eano = e_ano if e_ano == "90" | e_ano == "91" | e_ano == "96"
replace e_ano = eano
drop eano

gen emes =  e_mes if e_mes >= "01" & e_mes <= "12"
replace e_mes = emes
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*
/*
wgt1_kgs        byte   %10.0g                 Weight of child (kg - integer)
wgt2_kgs        byte   %10.0g                 Weight of child (kg - decimal)
*/
recode  wgt1_kgs 99=.

gen dot ="."
tostring wgt1_kgs wgt2_kgs, replace
egen peso = concat(wgt1_kgs dot wgt2_kgs) if wgt1_kgs !="."  | wgt2_kgs !="."
replace peso = ".9" if peso == "..9"
...
/*
lgth1_cm        int    %10.0g                 Length of child (cm - integer)
lgth2_cm        byte   %10.0g                 Length of child (cm - decimal)
*/
recode lgth1_cm  999=.
tostring lgth1_cm lgth2_cm , replace 
egen altura = concat(lgth1_cm dot lgth2_cm) if lgth1_cm !="."  | lgth2_cm !="."
replace altura = ".9" if altura == "..9"
...
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition90") sheetmodify cell(B13)   
restore

 preserve
collapse (max) *res [iw=factor], by(residence)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition90") sheetmodify cell(B14) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition90") sheetmodify cell(B16) 
restore

preserve
collapse (max) *quint [iw=factor], by(quintil)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition90") sheetmodify cell(B24) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition90") sheetmodify cell(B32) firstrow(variables)
restore
*/
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition90") sheetmodify cell(B29) 
restore
