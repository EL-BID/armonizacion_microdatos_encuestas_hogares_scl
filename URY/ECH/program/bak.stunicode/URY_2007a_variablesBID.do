...
** Ajustado Jul, 2017 por Iv�n Bornacelly SLC/EDU

*Replicando lo que est� en la progrmaci�n de a�os posteriores.
*NOTA: No hay categoria ENSE�ANZA T�CNICA para la variable de asistencia. Esta categor�a si existe para el caso de lso

*Para quienes est�n asistiendo.[e_52] - Si grupo de variables tiene 1 quiere decir que la persona asiste al nivel se�alado. De lo contrario no asiste a ese nivel. 
...
gen a_profbas=1 if e52_7>=1 // Esta podr�a ser equivalente a Ense�anza T�cnica
...
replace aedu_ci= e52_7 + 12   if a_profbas==1 // Esto podr�a ser equivalente a Ense�anza T�cnica.
...
*replace aedu_ci=0             if e50==2 & (edad>=5 & edad!=.)  <<------- Revisar si esto ir�a ac� // No CREO

replace aedu_ci=aedu_ci-1 if aedu_ci>=1 & aedu_ci!=.

*Para quienes no asisten esta variable se completa con la pregunta: �cu�l es el nivel y a�o m�s alto aprobado? [e_54]
...
gen fpbas  = (e52_7>0) // Formaci�n profesional b�sica
gen abacht = (e52_8>0)
gen amag   = (e52_9>0)
gen auniv  = (e52_10>0)
gen aterc  = (e52_11>0)
...
gen aedu_ci = .
replace aedu_ci = 0         if aprees==1 
replace aedu_ci = .         if aprie==1e52 // No se incluye. Equivale a educaci�n especial.
replace aedu_ci = e52_3     if apric==1  & e52_3<=6
replace aedu_ci = e52_4+6   if alic==1   & e52_4<=3
replace aedu_ci = e52_5+6   if acicb==1  & e52_5<=3
replace aedu_ci = e52_6+9   if abachs==1 & e52_6<=3
replace aedu_ci = .         if fpbas==1  // No se incluye. Corresponde a educaci�n para adultos
replace aedu_ci = e52_8+9   if abacht==1 & e52_8<=3
replace aedu_ci = e52_9+12  if amag==1   & e52_9<9
replace aedu_ci = e52_10+12 if auniv==1  & e52_10<9
replace aedu_ci = e52_11+12 if aterc==1  & e52_11<9
...
gen dtec = 1 if (e54_3_1>0) // Ense�anza t�cnica
...
replace aedu_ci = e54_6_1 + 12		if dter==1 & e54_6_1 <9
replace aedu_ci = e54_7_1 + 17		if dmae==1 & e54_7_1 <9
replace aedu_ci=. 		            if dtec==1 // No se incluye la educaci�n para adultos
replace aedu_ci = 0                 if e51==2 & (edad>=5 & edad!=.)  // Para quienes nunca asistieron y no est�n asistiendo.
*/
