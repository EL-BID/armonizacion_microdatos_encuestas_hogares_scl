...
Países: Colombia, Paraguay, Perú, Argentina, República Dominicana, Ecuador, Brasil
Autora: Carolina Hernández para 2020 (jhernandez@colmex.mx)
...
drop __000000 __000001 __000002
***************
***region_c ***
***************
...
g zona_c = 0 if clase == 2
replace zona_c = 1 if inlist(clase,1,.)
...
**********************************************************************
*El mes de marzo no tuvo los capítulos de desocupados e inactivos
*en las cabeceras pero se pueden deducir las personas que debieron quedar clasificadas en estos capíyulos con las variables del capítulo fuerza de trabajo
**********************************************************************
replace dsi=1 if p6351==1

replace ini=1 if p6240==5
replace ini=1 if p6300==2
replace ini=1 if inlist(p6310,9,10,11,12,13)
...
***************************
	...
	g rama_ci = .
	replace rama_ci = 1 if rama2d_r4 >=  1 & rama2d_r4 <= 3
	replace rama_ci = 2 if rama2d_r4 >= 5 & rama2d_r4 <= 9
	replace rama_ci = 3 if rama2d_r4 >= 10 & rama2d_r4 <= 33
	replace rama_ci = 4 if rama2d_r4 >= 35 & rama2d_r4 <= 39
	replace rama_ci = 5 if rama2d_r4 >= 41 & rama2d_r4 <= 43
	replace rama_ci = 6 if rama2d_r4 >= 45 & rama2d_r4 <= 47
	replace rama_ci = 7 if rama2d_r4 >= 55 & rama2d_r4 <= 56
	replace rama_ci = 8 if rama2d_r4 >= 49 & rama2d_r4 <= 53
	replace rama_ci = 9 if rama2d_r4 >= 58 & rama2d_r4 <= 63
	replace rama_ci = 10 if rama2d_r4 >= 64 & rama2d_r4 <= 66
	replace rama_ci = 11 if rama2d_r4 == 68
	replace rama_ci = 12 if rama2d_r4 >= 69 & rama2d_r4 <= 82
	replace rama_ci = 13 if rama2d_r4 >= 84 & rama2d_r4 <= 88
	replace rama_ci = 14 if rama2d_r4 >= 90 & rama2d_r4 <= 99
	replace rama_ci = 15 if rama2d_r4 == 0
	replace rama_ci = . if emp_ci == 0

	
	la var rama_ci "Rama de actividad"
	la def rama_ci 	1 "Agricultura, pesca, ganadería, caza y silvicultura"				                                                         ///
				   ...
				   14 "Actividades artísticas, entretenimiento, recreación y otras actividades de servicios"                        ///  
				   15 "No informa"                                                                                                  
...
*Para mayo, junio y julio de 2020 la recolección en centros poblados no fue afectada
*pero sí la recolección en cabeceras, el formulario reducido sólo permitió obtenr info de salario en ingresos con las variables:
...
*Modifico la info de ingresos con esta info


/*1. Ingreso monetario primera actividad (IMPA): Ingreso monetario de la primera actividad antes de imputación
  2. (IMPAES):Ingreso monetario de la primera actividad imputado (sólo para faltantes, extremos)
....ahora redefinido con las variables p6500 y p6750 con motivo del formulario reducido*/

***************
***ylmpri_ci***
***************
    gen ylmpri_ci=p6500 if inlist(p6430,1,2,3,8)
    replace ylmpri_ci=p6750 if inlist(p6430,4,5,9)
    replace ylmpri_ci=0 if inlist(p6430,6,7)
...
***ylnmpri_ci***
****************
     gen ylnmpri_ci =. /*No disponible para el periodo por formulario reducido */
...
***ylmsec_ci***
***************
    gen ylmsec_ci = . /*No disponible para el periodo por formulario reducido */
...
*****************
    ...
    gen ylmotros_ci= . /*No disponible para el periodo por formulario reducido */
...
************
    ...
    gen ylm_ci = ylmpri_ci /*Único disponible para el periodo por formulario reducido */
...
gen ingreso=p6500 if inlist(p6430,1,2,3,8)
replace ingreso=p6750 if inlist(p6430,4,5,9)
replace ingreso=0 if inlist(p6430,6,7)
...
****************
    ...
    g remesas_ci = . /*No disponible para el periodo por formulario reducido */
...
***remesas_ch***
****************
    g remesas_ch = . /*No disponible para el periodo por formulario reducido */
...
*************
	...
	la def des2_ch 	0 "No tiene servicio sanitario" 								///
					1 "Conectado a red general, cámara séptica, pozo o letrina" 	///
