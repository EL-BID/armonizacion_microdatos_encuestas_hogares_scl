...
label define mes_c 9 "Septiembre" 10 "Octubre" 11 "Noviembre", add modify
...
************************************
gen byte causa=real(causa_no)  /*en ultimo mes*/
gen byte trab=real(trab_m_p) /*en ultimo mes*/
...
****************
generat condocup_ci=.
replace condocup_ci=1 if trab_m_p=="1" 
replace condocup_ci=1 if (trab_m_p=="2"  & (causa_no_t>="1"  & causa_no_t<="10")) 
replace condocup_ci=2 if (trab_m_p=="2"  & (causa_no_t=="11")) 
...
*lpe_ci***
*********
generat lpe_ci =16795.52 if zona_c==1
replace lpe_ci =12475.06 if zona_c==0
label var lpe_ci "linea de pobreza alimentaria oficial del pais"

*********
*lp2_ci***
*********
generat lp2_ci =20599.71 if zona_c==1
replace lp2_ci =14749.27 if zona_c==0
label var lp2_ci "linea de pobreza de capacidades"

***********
*lp_ci ***
***********
generat lp_ci =33698.54  if zona_c==1
replace lp_ci =22637.25  if zona_c==0
...
*rename municipio ubica_geo
/*
Nota: No se debe eliminar este archivo. Asigna los salarios Minimos legales
*/	
...
capture gen municipio= ubica_geo
gen zona_salmm=.	
...
generat salmm_ci=13330.00*30 if zona_salmm==1
...
gen horaspri_ci=hr_seman if emp3_ci==1 

*****************
***horastot_ci***
*****************

gen horastot_ci=hr_seman+hr_sem_s  if emp3_ci==1 
...
replace categopri_ci=3 if posicion=="1" | posicion=="2" | posicion=="7" | posicion=="8" 
replace categopri_ci=4 if posicion=="6" 
...
gen categosec_ci=.
replace categosec_ci=1 if posi_sec=="3" | posi_sec=="4"
replace categosec_ci=2 if posi_sec=="5" 
replace categosec_ci=3 if posi_sec=="1" | posi_sec=="2" | posi_sec=="7" | posi_sec=="8" 
replace categosec_ci=4 if posi_sec=="6" 
...
replace tipocontrato_ci=3 if contrato>="3" & contrato<="5" 
...
gen nempleos_ci=0
replace nempleos_ci=1 if num_empl==1
replace nempleos_ci=2 if num_empl>=2
...
***tamfirma_ci***
*****************
destring tot_trab, replace
gen tamfirma_ci=.
replace tamfirma_ci=0 if tot_trab~=2
replace tamfirma_ci=1 if tot_trab>=4
replace tamfirma_ci=. if tot_trab ==.

/*
gen firmapeq_ci=.
replace firmapeq_ci=1 if tot_trab==1 | tot_trab==2
recode firmapeq_ci .=0 if tot_trab>2 & tot_trab!=.
*/
*****************
***spublico_ci***
*****************
...
gen spublico_ci=.
replace spublico_ci=1 if tipo_emp =="1" & emp3_ci==1
replace spublico_ci=0 if tipo_emp ~="1" & emp3_ci==1
...
gen ocupa_ci=.
replace ocupa_ci=1 if (ocupa==11 | ocupa==12) & emp3_ci==1
replace ocupa_ci=2 if (ocupa==21) & emp3_ci==1
replace ocupa_ci=3 if (ocupa==51 | ocupa==61 |ocupa==62) & emp3_ci==1
replace ocupa_ci=4 if (ocupa==71 | ocupa==72) & emp3_ci==1
replace ocupa_ci=5 if (ocupa>13 & ocupa<=14 | ocupa==81) & emp3_ci==1
replace ocupa_ci=6 if (ocupa==41) & emp3_ci==1
replace ocupa_ci=7 if (ocupa>=52 & ocupa<=55) & emp3_ci==1
replace ocupa_ci=8 if (ocupa==83) & emp3_ci==1
replace ocupa_ci=9 if (ocupa==82| ocupa==99) & emp3_ci==1
replace ocupa_ci=. if emp3_ci~=1

* Modificacion MGD 07/07/2014: reclasificacion de algunos items.
gen ocupa_ci1=.
replace ocupa_ci1=1 if (ocupa>=11 & ocupa<=12) & emp_ci==1
replace ocupa_ci1=2 if (ocupa==21) & emp_ci==1
replace ocupa_ci1=3 if (ocupa==51 | ocupa==61 |ocupa==62) & emp_ci==1
replace ocupa_ci1=4 if (ocupa==71 | ocupa==72) & emp_ci==1
replace ocupa_ci1=5 if (ocupa==13 |ocupa==14 | ocupa==81 | ocupa==82 |ocupa==83) & emp_ci==1
replace ocupa_ci1=6 if (ocupa==41) & emp_ci==1
replace ocupa_ci1=7 if (ocupa>=52 & ocupa<=55) & emp_ci==1
replace ocupa_ci1=8 if (ocupacio=="8310" |ocupacio=="8311" | ocupacio=="8312") & emp_ci==1
replace ocupa_ci1=9 if (ocupa==99 | ocupa==98) & emp_ci==1
...
***categoinac_ci***
*******************
gen categoinac_ci =1 if (causa_no_t=="14" & condocup_ci==3)
...
gen nrylmpri_ci=(ylmpri_ci==. & emp3_ci==1)
...
gen ylmsec_ci=ing2_ml if emp3_ci==1 
...
gen ylnm_ci=ing_nml if emp3_ci==1
...
*	asispre_ci
******************************
*Variable agregada por Ángela López - 08/31/2018
	g asispre_ci=.
...
********************************************
destring agua luz bano_excus bano_ex_ho drenaje pisos muros techos cu_us_do,replace
destring num_cuarto cu_cocina cu_co_do telefono carac_viv tenencia,replace
***************
**aguared_ch***
***************
gen aguared_ch=.
...
gen bano_ch=.
replace bano_ch=1 if bano_excus==1
replace bano_ch=0 if bano_excus==2
...
gen banoex_ch=.
replace banoex_ch=1 if bano_ex_ho==1
replace banoex_ch=0 if bano_ex_ho==2
...
***banomejorado_ch***
*********************
g       banomejorado_ch = 1 if (bano_excus ==1 & (drenaje>=1 &  drenaje<=2)  & bano_ex_ho ==1)
replace banomejorado_ch = 0 if (bano_excus ==1 & (drenaje>=1 &  drenaje<=2)  & bano_ex_ho ==2) | (bano_excus ==1 & (drenaje>=3 &  drenaje<=4)) | bano_excus ==2
...
local linorig lp_ci lp2_ci lpe_ci
foreach e of local linorig {
replace `e' = `e'/100
}
