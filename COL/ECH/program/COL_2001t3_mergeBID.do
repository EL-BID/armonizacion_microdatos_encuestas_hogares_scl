...
*** VIVIENDA
forvalues i=7/9 {
infile using "`m`i''col01_T01`zona'.dct", clear
keep if `x'TIPOD=="01"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 
saveold `m`i''col`zona'01vivi.dta,  replace
}

use "`m7'col`zona'01vivi.dta", clear
append using "`m8'col`zona'01vivi.dta"
append using "`m9'col`zona'01vivi.dta"
sort  `x'IDENT mes_c1 
saveold `t3'col`zona'01vivi.dta, replace


*** EDUCACION
forvalues i=7/9 {
infile using "`m`i''col01_T10`zona'.dct", clear
keep if `x'TIPOD=="10"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 
saveold `m`i''col`zona'01edu.dta,  replace
}
use "`m7'col`zona'01edu.dta", clear
append using "`m8'col`zona'01edu.dta"
append using "`m9'col`zona'01edu.dta"
sort  `x'IDENT mes_c1 
saveold "`t3'col`zona'01edu.dta", replace

*** FUERZA DE TRABAJO
forvalues i=7/9 {
infile using "`m`i''col01_T50`zona'.dct", clear
keep if `x'TIPOD=="50"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 `x'1_T50_ 
saveold `m`i''col`zona'01ft.dta,  replace
}
use "`m7'col`zona'01ft.dta", clear
append using "`m8'col`zona'01ft.dta"
append using "`m9'col`zona'01ft.dta"
sort  `x'IDENT mes_c1 `x'1_T50_
saveold "`t3'col`zona'01ft.dta", replace

*** OCUPADOS EMPLEO PRINCIPAL
forvalues i=7/9 {
infile using "`m`i''col01_T60`zona'.dct", clear
keep if `x'TIPOD=="60"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 `x'1_T60_
saveold `m`i''col`zona'01oep.dta,  replace
}

use "`m7'col`zona'01oep.dta", clear
append using "`m8'col`zona'01oep.dta"
append using "`m9'col`zona'01oep.dta"
sort  `x'IDENT mes_c1 `x'1_T60_
saveold "`t3'col`zona'01oep.dta", replace

*** OCUPADOS SUBEMPLEO
forvalues i=7/9 {
infile using "`m`i''col01_T61`zona'.dct", clear
keep if `x'TIPOD=="61"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 `x'1_T61_
saveold `m`i''col`zona'01os.dta,  replace
}
use "`m7'col`zona'01os.dta", clear
append using "`m8'col`zona'01os.dta"
append using "`m9'col`zona'01os.dta"
sort  `x'IDENT mes_c1 `x'1_T61_
saveold "`t3'col`zona'01os.dta", replace

*** OCUPADOS EMPLEO SECUNDARIOS
forvalues i=7/9 {
infile using "`m`i''col01_T62`zona'.dct", clear
keep if `x'TIPOD=="62"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 `x'1_T62_
saveold `m`i''col`zona'01oes.dta,  replace
}
use "`m7'col`zona'01oes.dta", clear
append using "`m8'col`zona'01oes.dta"
append using "`m9'col`zona'01oes.dta"
sort  `x'IDENT mes_c1 `x'1_T62_
saveold "`t3'col`zona'01oes.dta", replace

*** DESOCUPADOS 
forvalues i=7/9 {
infile using "`m`i''col01_T70`zona'.dct", clear
keep if `x'TIPOD=="70"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 `x'1_T70_ 
saveold `m`i''col`zona'01des.dta,  replace
}
use "`m7'col`zona'01des.dta", clear
append using "`m8'col`zona'01des.dta"
append using "`m9'col`zona'01des.dta"
sort  `x'IDENT mes_c1 `x'1_T70_ 
saveold "`t3'col`zona'01des.dta", replace

*** INACTIVOS
forvalues i=7/9 {
infile using "`m`i''col01_T80`zona'.dct", clear
keep if `x'TIPOD=="80"
gen mes_c1=`i'
sort  `x'IDENT mes_c1 `x'1_T80_
saveold `m`i''col`zona'01ina.dta,  replace
}
use "`m7'col`zona'01ina.dta", clear
append using "`m8'col`zona'01ina.dta"
append using "`m9'col`zona'01ina.dta"
sort  `x'IDENT mes_c1 `x'1_T80_
saveold "`t3'col`zona'01ina.dta", replace
*/

*2 - Merge de los diferentes modulos
*-----------------------------------
use `t3'col`zona'01vivi.dta, clear
merge `x'IDENT mes_c1 using `t3'col`zona'01edu.dta,  
tab _merge
drop _merge
rename `x'1_T10_ `x'1_T50_
sort  `x'IDENT mes_c1 `x'1_T50_

merge `x'IDENT mes_c1 `x'1_T50_ using `t3'col`zona'01ft.dta
tab _merge
drop _merge
rename `x'1_T50_ `x'1_T60_
sort  `x'IDENT mes_c1 `x'1_T60_

merge `x'IDENT mes_c1 `x'1_T60_ using `t3'col`zona'01oep.dta
tab _merge
drop _merge
rename `x'1_T60_ `x'1_T61_
sort  `x'IDENT mes_c1 `x'1_T61_

merge `x'IDENT mes_c1 `x'1_T61_ using `t3'col`zona'01os.dta
tab _merge
drop _merge
rename `x'1_T61_ `x'1_T62_
sort  `x'IDENT mes_c1 `x'1_T62_

merge `x'IDENT mes_c1 `x'1_T62_ using `t3'col`zona'01oes.dta
tab _merge
drop _merge
rename `x'1_T62_ `x'1_T70_
sort  `x'IDENT mes_c1 `x'1_T70_

merge `x'IDENT mes_c1 `x'1_T70_ using `t3'col`zona'01des.dta
tab _merge
drop _merge
rename `x'1_T70_ `x'1_T80_
sort  `x'IDENT mes_c1 `x'1_T80_

merge `x'IDENT mes_c1 `x'1_T80_ using `t3'col`zona'01ina.dta
tab _merge
drop _merge
...
ren factor  `x'FACTEXP
	...
	rename	`x'6A_T101	p6A_T101
	rename	`x'6A_T102	p6A_T102
	rename	`x'6B_T101	p6B_T101
	rename	`x'6B_T102	p6B_T102
	rename	`x'6C_T10	p6C_T10
	rename	`x'6D_T101	p6D_T101
	rename	`x'6D_T102	p6D_T102
	rename	`x'6E_T10	p6E_T10
	rename	`x'6F_T10	p6F_T10
	rename	`x'6G_T10	p6G_T10
	rename	`x'6H_T10	p6H_T10
...
cap	rename	`x'46_T6210	p46_T6210
