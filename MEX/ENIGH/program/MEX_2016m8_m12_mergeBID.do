...
global ruta = "\\Sdssrv03\surveys\\survey\MEX\ENIGH\2016\m8_m12\data_orig"
...
Última versión: Daniela Zuluaga, E-mail: danielazu@iadb.org, da.zuluaga@hotmail.com
...
replace ing_1=(ing_1/dic16)/12 if clave=="P009" | clave=="P016"
...
saveold "$ruta\ingreso_deflactado16_per.dta", replace
...
save "$ruta\ingresonomonetario_def16.dta", replace

use "$ruta\ingresonomonetario_def16.dta", clear
...
save "$ruta\esp_def16.dta", replace
...
save "$ruta\reg_def16.dta", replace
...
merge proyecto folioviv foliohog using "$ruta\ingreso_deflactado16.dta"
tab _merge
drop _merge*/
...
merge 1:1 folioviv foliohog using "$ruta\esp_def16.dta"
tab _merge
drop _merge
...
merge 1:1 folioviv foliohog using "$ruta\reg_def16.dta"
tab _merge
drop _merge
...
save "$ruta\ingresotot16.dta", replace

saveold "$ruta\gtos_autoc16.dta", replace
...
use "$ruta\trabajos.dta",clear
	...
	*Daniela Zuluaga Enero 2018: Dado que no me permite hacer el merge como estaba anteriormente renombro las siguientes variables
	rename pres_21 pres_211
	rename pres_22 pres_222
	...
	rename pres_2111 pres_211
	rename pres_2112 pres_212
	rename pres_2221 pres_221
	rename pres_2222 pres_222
	...
	pres_211 pres_221 pres_231 pres_241 pres_251 pres_261 {
	...
	pres_212 pres_222 pres_232 pres_242 pres_252 pres_262  {
...
merge 1:1 folio numren using "$ruta\ingreso_deflactado16_per.dta", keep (match master)
...
merge m:1 folioviv foliohog using "$ruta\gtos_autoc16.dta", keep (match master)
drop _merge
