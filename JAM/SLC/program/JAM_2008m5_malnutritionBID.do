...
*->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<-*
clear all
use "${surveysFolder}\survey\JAM\SLC\2008\m5\data_merge\JAM_2008m5.dta"
set more off
*Main directory
...
replace e_ano = 2008 if e_ano ==8
replace e_ano = 2009 if e_ano ==9

gen emes =  e_mes if e_mes >= 2 & e_mes <= 9
replace e_mes = emes

gen edia =  e_dia if e_dia >=1 & e_dia <= 31
replace e_dia = edia


destring e_ano e_mes e_dia , replace
...
*replace e_ano = 2000+e_ano
 ...
 replace anios = 200 if  (b_year >=0 & b_year <=9)
...
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition08") sheetmodify cell(B13)   
restore

 preserve
collapse (max) *res [iw=factor], by(residence)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition08") sheetmodify cell(B14) 
restore

 preserve
 *elimino la variable original mortgage temporalmente, ya que me da problemas en el collapse
 capture drop mortgage
 rename ageyrs edadanios
 rename agemth edadmeses
 destring *age, replace
collapse (max) *age [iw=factor], by(amonth)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition08") sheetmodify cell(B16) 
restore

preserve
collapse (max) *quint [iw=factor], by(quintil)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition08") sheetmodify cell(B24) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition08") sheetmodify cell(B32) firstrow(variables)
restore
*/
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition08") sheetmodify cell(B29) 
restore
