...
notes: Para las variables de educacion no se considera educacion especial.
...
replace aedu_ci=10 if r204==3 & (r205<10|r205>=12)

*sup univ  
replace aedu_ci=12+r205 if r204==4 & (r205>=1 & r205<=15)

*sup no univ 
replace aedu_ci=12+r205 if r204==5 & (r205>=1 & r205<=4)/*one h in 4*/

* MGR Aug, 2015: desactivo para mantener sintaxis homogenea con años anteriores donde no se resta 1
*restar 1 para otbtener los anios de educacion completados
...
*no asisten pero asistieron-max nivel alcanzado
replace aedu_ci=0 if (r217a==0 | r217a==1 ) & r215==1 
replace aedu_ci=r217b if r217a==2 & (r217b>=1 & r217b<=9)
replace aedu_ci=9 if r217a==2 & (r217b>=10)
replace aedu_ci=10 if r217a==3 & (r217b>=1 & r217b<10)
replace aedu_ci=r217b if r217a==3 & (r217b>=10 & r217b<=12)
replace aedu_ci=12+r217b if r217a==4 & (r217b>=1 & r217b<=15)
replace aedu_ci=12+r217b if r217a==5 & (r217b>=2 & r217b<=3)
...
*	edupre_ci
******************************
g byte edupre_ci=(r204==0 |r204==1) 
...
label variable repiteult_ci "Personas que han repetido el ultimo grado o curso"
******************************
*	edupub_ci 
******************************
g edupub_ci=.
...
*	aguared_ch
******************************
g aguared_ch=(r312==1 | r312==2)

******************************
*	aguadist_ch
******************************
/*g aguadist_ch=.
replace aguadist_ch=1 if r312==1
replace aguadist_ch=2 if r312==2
replace aguadist_ch=3 if 

the last line cannot be codified (includes those households that do not have piped water
...
*	aguamala_ch
******************************
*Yanira O. La variable se hace missing pues no es posible construir información comparable.
*Sin embargo, se guarda la programación anterior
*g aguamala_ch=(r315==1)
gen aguamala_ch=.

/*le hacen algun tratamiento al agua*/
******************************
*	aguamide_ch
******************************
g aguamide_ch=.
...
*	bano_ch & banoex_ch
******************************
g bano_ch=(r319>=1 & r319<=7)
label var bano_ch "Inodoro o letrina"

*Yanira O.  Se había seleccionado la opción equivocada se cambia 1 por 2
gen banoex_ch=(r321==2) if bano_ch==1
******************************
*	des1_ch
******************************
g des1_ch=.
...
***aguamejorada_ch***
*********************
g       aguamejorada_ch = 1 if  (r312 >=1 & r312 <=2) | (r313 >=4 & r313 <=5) | r313 == 8 | r313 == 10
replace aguamejorada_ch = 0 if ((r312 >=3 & r312 <=4) & (r313 != 4 | r313 != 5 | r313 != 8 | r313 != 10) ) | (r313 >=1 & r313 <=3) | (r313 >=6 & r313 <=7) | r313 == 9 | (r313 >=11 & r313 <=12)
		
*********************
***banomejorado_ch***
*********************
g       banomejorado_ch = 1 if  ((r319>=1 & r319 <=2) | r319 == 5 | r319 == 7) & r321 ==2
replace banomejorado_ch = 0 if (((r319>=1 & r319 <=2) | r319 == 5 | r319 == 7) & r321 ==1) | (r319>=3 & r319 <=4) | r319 == 6 | r318 ==5
