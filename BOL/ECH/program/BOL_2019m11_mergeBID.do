...
global in = "${surveysFolder}\survey\BOL\ECH\2019\m11\data_orig\"
global out = "${surveysFolder}\survey\BOL\ECH\2019\m11\data_merge\"
...
Última modificación: Lina Arias (06/25/2020)
...
use "$in\eh2019_equipamiento.dta", clear
...
label var `x'_2 "Catre"
label var `x'_3 "cocina (a gas, eléctrica, etc.)"
label var `x'_4 "horno microondas/ microondas"
label var `x'_5 "refrigerador o freezer"
label var `x'_6 "Aire acondicionado/calefa"
label var `x'_7 "computadora (laptop o tablet pc, etc)"
label var `x'_8 "radio o radiograbador"
label var `x'_9 "minicomponente o equipo de sonido"
label var `x'_10 "televisor convencional"
label var `x'_11 "televisor plasma"
label var `x'_12 "lavadora de ropa"
label var `x'_13 "Bicicleta"
label var `x'_14 "motocicleta (para uso del hogar)"
label var `x'_15 "automóvil (para uso del hogar)"
}

duplicates report folio
		  
sort folio
save "$in\eh2019_gastos_equipamiento_reshape.dta", replace

use "$in\eh2019_gastosalimentarios.dta", clear
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
...
duplicates report folio
sort folio
save "$in\eh2019_gastos_alimentarios_reshape.dta", replace

use "$in\eh2019_persona.dta", clear
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
...
use "$in\eh2019_vivienda.dta", clear
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
...
use "$in\eh2019_gastosnoalimentarios", clear
*Modificación Cesar Lins - Feb 2021, data was updated by INE and some variable names changed to UPPERCASE
rename *, lower
...
merge m:1 folio using "$in\eh2019_vivienda.dta", force
drop _merge

merge m:1 folio using "$in\eh2019_gastos_equipamiento_reshape.dta", force
drop _merge

merge m:1 folio using "$in\eh2019_gastos_alimentarios_reshape.dta", force
drop _merge

merge m:1 folio using "$in\eh2019_gastosnoalimentarios", force
drop _merge

*Modificación Cesar Lins - Feb 2021 / saveold didn't work because labels are too long
save "$out\BOL_2019m11.dta", replace
