*Elaboraci�n: Mar�a Laura Oliveri (mloliveri@iadb.org / lauraoliveri@yahoo.com)
*Diciembre, 2015
*Última modificación: Alvaro Altamirano (alvaroalt@iadb.org) Julio 2017.
...
global dicc "$ruta\survey\BRA\NPME/`anio'/a\docs\documentacao\Layout\"


set more off
* levanta base usando diccionario

local mes "01 02 03 04 05 06 07 08 09 10 11 12" 
foreach i of local mes {
infile using "$dicc\input.do", using("`base_in'/`anio'/m`i'/data_orig/PMEnova.`i'`anio'.txt")
save "`base_in'/`anio'/m`i'/data_orig/PMEnova.`i'`anio'.dta", replace
clear
}
