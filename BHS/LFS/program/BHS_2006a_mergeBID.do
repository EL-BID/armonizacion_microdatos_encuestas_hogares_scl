...
global ruta = "\\Sdssrv03\surveys\\survey\\BHS\LFS\\2006\\a\\data_orig"
...
use  "$ruta\Bahamas_LFS_2006_housing.dta", clear
capture drop sup_dist2
gen sup_dist2  = string(sup_dist,"%03.0f")
capture drop id
egen id=concat(island sup_dist2 enum_dist hhno)
sort id
save "$ruta\Bahamas_LFS_2006_housing1.dta", replace

use "$ruta\Bahamas_LFS_2006_individual.dta", clear
gen sup_dist2  = string(sup_dist,"%03.0f")
egen id=concat(island sup_dist2 enum_dist hhno)
sort id
merge id using "$ruta\Bahamas_LFS_2006_housing1.dta"
tab _merge
keep if _merge==3
drop _merge
