...
use `base_in', clear
/*
*1.No encuentro la variable ocupacio  y tampoco el diccionario de las ocupaciones
asi que hay que tener cuidado con  la interpretacion de la misma porque puede estar errada
lo que hago es utilizar la variable que SI eciste y tratar de conservar la idea anterior
Codigo anterior:
...
2. emp 
gen trab=(trabajo1>1 & trabajo1<=4), es hasta tres
replace trab=2 if (trabajo1==5 | trabajo1==6)

gen byte emp_ci=(trab==1)
replace emp_ci=1 if cateprin>0 
*/
...
***ocupa_ci***
**************
*No encuentro el diccionario de esta variable existe ocuprin pero no se como se codifica
...
gen ysec1=ysec
replace ysec1=. if ysec==0 /*No aplicable*/
...
gen `x'1=`x'
replace `x'1=. if `x'==0 /*No aplicable*/
}
