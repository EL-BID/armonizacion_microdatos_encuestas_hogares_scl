...
capture drop lp_ci
gen lp_ci = .
replace lp_ci =      13182 if zona_c==1
replace lp_ci =      10319 if zona_c==0
...
gen lpe_ci = .
replace lpe_ci =       6047 if zona_c==1
replace lpe_ci =       5238 if zona_c==0
...
****************
gen afiliado_ci=.
replace afiliado_ci=1 if asegura>=1 & asegura<=3 |(afilpenc>=1 & afilpenc<=6)
...
recode condocup_ci .=3 if condact>=8 & condact<=14 & edad_ci>=10
recode condocup_ci .=4 if  condact==0 | edad_ci<10
...
gen cesante_ci=.
replace cesante_ci=1 if experien==1 & condocup_ci==2
...
replace aux_p = ypension/2 if per06==2
replace aux_p = ypension/3 if per06==3
replace aux_p = ypension/4 if per06==4
replace aux_p = ypension/6 if per06==6
replace aux_p = ypension/12 if per06==7
replace aux_p = . if ypension==99999999*/
...
gen aux_ps=.
/*
gen aux_ps= c35b if c35b1==1

replace aux_ps = c35b/2 if c35b1==2
replace aux_ps = c35b/3 if c35b1==3
replace aux_ps = c35b/4 if c35b1==4
replace aux_ps = c35b/6 if c35b1==6
replace aux_ps = c35b/12 if c35b1==7
...
* 1995 junio (fuente OIT-CEPAL observatorio de la crisis)
gen salmm_ci= 29458
...
gen desalent_ci=(emp_ci==0 & (pqnobus==4 | pqnobus==5)) & edad>=12
...
***subemp_ci***
***************
gen subemp_ci=(horaspri_ci<=30 & trabmas==1) & emp_ci==1

*******************
***tiempoparc_ci***
*******************

gen tiempoparc_ci=(horastot_ci<=30 & trabmas==2)
...
gen categopri_ci=.
replace categopri_ci=1 if categpr==1
replace categopri_ci=2 if categpr==2 
replace categopri_ci=3 if categpr==3 | categpr==4 |categpr==5 
replace categopri_ci=4 if categpr==6 
...
replace categosec_ci=4 if categse==6 
...
gen spublico_ci=(secintpr>=1 & secintpr<=5)
...
replace rama_ci=2 if ramapr>=2100 & ramapr<=2909 & emp_ci==1
...
replace rama_ci=9 if ramapr>=9100 & ramapr<=9999 & emp_ci==1
...
gen durades1_ci=1 if tpobusq>=1 & tpobusq<=2
replace durades1_ci=2 if tpobusq==3
replace durades1_ci=3 if tpobusq==4
replace durades1_ci=4 if tpobusq==5
replace durades1_ci=. if tpobusq==9
...
gen durades_ci=.
*menos de 2 meses
replace durades_ci=(1+2)/2 if durinact==1
 ...
 * Nota MGD 08/04/2014: Calculo de pormedios para la duracion utilizando la variable de tiempo de busqueda de empleo.
gen durades_ci=.
*menor a un mes
replace durades_ci=(1+4.3)/2/4.3 if tpobusq ==1
...
/*
RELACION DE PARENTESCO	(parentco)
 1   Jefe(a)
 2   Esposo(a)
 3   Hijo
 4   Yerno o Nuera
...
*/
 ...
 replace incl=0 if (parentco>=8 & parentco<=9 )
...
*/
 ...
 gen 	 area=1 if (zona==1 | zona==2)
 replace area=2 if (zona==3 | zona==4)
...
** ID Hogar
 ...
 rename id_hogar id_hogar_orig
 
 gen y=1 if nrolinea==1
 gen id_hogar=sum(y)

 gen prueba= id_hogar-id_hogar_orig
 tab prueba
 
** Dwelling ID

 gen x=1 if nrolinea==1 & hogar==1
 gen id_viv=sum(x)
...
rename anoest anoest_orig
 ...
 gen	 anoest=0  if niveduc==0  | niveduc==1  | niveduc==2
 ...
 replace anoest=10 if niveduc==24 | niveduc==34 
 replace anoest=11 if niveduc==25 | niveduc==35 
 replace anoest=12 if niveduc==41 | niveduc==51 | niveduc==36 
 replace anoest=13 if niveduc==42 | niveduc==52
 replace anoest=14 if niveduc==43 | niveduc==53 
 replace anoest=15 if niveduc==44 | niveduc==54
 replace anoest=16 if niveduc==55 
 replace anoest=17 if niveduc==56
 replace anoest=18 if niveduc==57 
 replace anoest=19 if niveduc==58
 replace anoest=99 if niveduc==99 | niveduc==. | niveduc==19 | niveduc==29  | niveduc==39 | niveduc==49 | niveduc==59 | edad<=4
...
*** GOAL 3 PROMOTE GENDER EQUALITY AND EMPOWER WOMEN
 ...
 gen ter=1 if   (asiste==4 & niveduc<=54) | (asiste==4 & niveduc<=43)
...
* Without domestic Service
 ...
 gen	 WENAS=0 if (edad>=15 & edad<=64) & (categ==3 | categ==4) & (ramar>=2 & ramar<=10) & peaa==1
 replace WENAS=1 if (edad>=15 & edad<=64) & (categ==3 | categ==4) & (ramar>=2 & ramar<=10) & sexo==2 & peaa==1

** Target 4, Indicator: Share of women in wage employment in the non-agricultural sector (%)

* With domestic servants

 gen	 WENASD=0 if (edad>=15 & edad<=64) & (categ==3 | categ==4 | categ==5) & (ramar>=2 & ramar<=10) & peaa==1
 replace WENASD=1 if (edad>=15 & edad<=64) & (categ==3 | categ==4 | categ==5) & (ramar>=2 & ramar<=10) & sexo==2 & peaa==1
...
** Disconnected Youths
 ...
 replace stu_sd=1 if (inactiv==3 | inactiv==4) & (edad>=15 & edad<=24)

 gen	 DISCONN=0 if (edad>=15 & edad<=24)
 replace DISCONN=1 if (edad>=15 & edad<=24) & ((inactiv==1| inactiv==6) | ((disptrab==3 | (pqnobus==4 | pqnobus==7))  & stu_sd==0))
