...
gen byte causa=real(substr(trabajo,4,2))  /*en ultimo mes*/
****************
****condocup_ci*
****************
gen byte no_traba=real(substr(trabajo,4,2)) 
...
replace condocup_ci=2 if (trab_rem=="2" & trab_srem=="2" & trab_espo=="2") & (no_traba>=11 & no_traba<=12)
...
replace cotizando_ci=1 if (presta1>=1 & presta1<=5)  & cotizando_ci==0
...
*lpe_ci***
*********
generat lpe_ci =388.81 if zona_c==1
replace lpe_ci =289.47  if zona_c==0
label var lpe_ci "linea de pobreza alimentaria oficial del pais"

*********
*lp2_ci***
*********
generat lp2_ci =476.87 if zona_c==1
replace lp2_ci =342.24  if zona_c==0
label var lp2_ci "linea de pobreza de capacidades"

*********
*lp_ci***
*********
generat lp_ci =780.10 if zona_c==1
replace lp_ci =525.27 if zona_c==0
...
generat salmm_ci=22.60*30 if zona_salmm==1
replace salmm_ci=20.95*30 if zona_salmm==2
replace salmm_ci=19.05*30 if zona_salmm==3
label var salmm_ci "Salario minimo legal"

*Se toma el salario establecido en abril de 1996
...
gen hr_seman=real(hrs_sem)
gen hr_sem_s=real(hrs_sec)
...
*****************
/*
gen contrato=substr(cont_sin,1,1) 
gen contrato_ci=.
...
gen num_empleo=substr(trabajo,6,1)
...
gen byte tot_tra1=real(numper)
replace tot_tra1=. if tot_tra1==99999
gen byte tamfirma_ci=.
replace tamfirma_ci=0 if tot_tra1<5
replace tamfirma_ci=1 if tot_tra1>=5
/*
gen firmapeq_ci=.
replace firmapeq_ci=1 if tot_tra1==1 | tot_tra1==2
recode firmapeq_ci .=0 if tot_tra1>2 & tot_tra1!=.
*/
*****************
***spublico_ci***
*****************
...
gen spublico_ci=.
replace spublico_ci=1 if (nom_empr=="1" | nom_empr=="3") & emp_ci==1
replace spublico_ci=0 if nom_empr~="1" & nom_empr ~="3" & emp_ci==1
...
gen yedc=.
replace yedc=0 if educab==0 | educab==1  /* no escuela o presc*/
replace yedc=1 if educab==2
replace yedc=2 if educab==3
replace yedc=3 if educab==4
replace yedc=4 if educab==5
replace yedc=5 if educab==6
replace yedc=6 if educab==7
replace yedc=7 if educab==8
replace yedc=8 if educab==9
replace yedc=9 if educab==10
replace yedc=11.5 if educab==11
replace yedc=12 if educab==12

/* Revision 31 JUL 2006 (victoria) */ 

/*PROGRAMACION VIEJA 
replace yedc=15 if educab==13
replace yedc=18 if educab==14
replace yedc=21 if educab==15 */ 

*PROGRAMACION NUEVA 
replace yedc=14 if educab==13
replace yedc=17 if educab==14
replace yedc=19 if educab==15 
...
/* No se tiene encuentra centros de alfabetizacion y educacion 
para adultos*/
/* BYTE REDONDEA LOS .5 HACIA ABAJO. 31 JUL 2006 SE MANTIENEN LOS .5 (victoria) 
...
********************************************
destring  pisos num_cua cua_coc cua_dor  tipo_viv tenencia,replace

***************
**aguared_ch***
***************

*gen aguared_ch=real(substr(agua,1,1))
*replace aguared_ch=0 if aguared_ch==2

*Modificación Mayra Sáenz - Agosto 2015
destring agua*, replace

gen aguared_ch=1 if agua17 ==1
replace aguared_ch=0 if agua17==2 | agua17==0

****************
**aguadist_ch***
****************
*Modificación Mayra Sáenz - Agosto 2015
*gen aguadist_ch=real(substr(agua,2,1))

gen aguadist_ch=.
replace aguadist_ch=1 if agua16==1 
replace aguadist_ch=2 if agua16==2


****************
**aguamala_ch***
****************
*Modificacion Mayra Sáenz - Agosto 2015 : antes se generaba como missing

gen aguamala_ch= 1     if (agua21 >=5 & agua21 <=6)
replace aguamala_ch= 0 if (agua21 >=1 & agua21 <=4)
...
*gen luz_ch=real(substr(com_luz,2,1))
*replace luz_ch=0 if luz_ch==2

*Modificacion Mayra Sáenz - Agosto 2015
destring luz33,replace
gen     luz_ch= 1 if luz33 ==1
replace luz_ch= 0 if luz33 ==2 | luz33 ==0
...
****************
/*
gen combust_ch=real(substr(com_luz,1,1))
...
*Modificacion Mayra Sáenz - Agosto 2015
destring combust, replace
gen combust_ch = 1     if combusti==4 | combusti==5
replace combust_ch = 0 if (combusti>=1 & combusti<=3) | (combusti>=6 & combusti<=7)
...
/*
gen bano_ch=real(substr(bano,1,1))
replace bano_ch=0 if bano_ch==2
*/

*Modificacion Mayra Sáenz - Agosto 2015
gen bano_ch=real(substr(bano23,1,1))
replace bano_ch=0 if bano_ch==2
...
*gen banoex_ch=real(substr(bano,4,1))
...
**Modificacion Mayra Sáenz - Agosto 2015
/*gen drenaje2=real(substr(drenaje,3,1))
gen des1_ch=.
...
replace des1_ch=1 if drenaje27==2 | drenaje27==3
replace des1_ch=2 if drenaje27==4 
replace des1_ch=3 if drenaje27==1 
...
****pared_ch****
****************
*gen muros1=real(substr(muros,1,2))


**Modificacion Mayra Sáenz - Agosto 2015
gen muros1=real(substr(muros08,1,2))
...
****techo_ch****
****************
*gen techo1=real(substr(techos,1,2))

**Modificacion Mayra Sáenz - Agosto 2015
gen techo1=real(substr(techos11,1,2))
...
/*gen basura=real(substr(basurafr,1,1))
...
g resid_ch =.
replace resid_ch=0 if (basura29_1==6 | basura29_2==6 | basura29_3==6)
replace resid_ch=1 if resid_ch ==. & (basura29_1==2 | basura29_2==2 | basura29_3==2 | basura29_1==4 | basura29_2==4 | basura29_3==4)
replace resid_ch=2 if resid_ch ==. & (basura29_1==1 | basura29_2==1 | basura29_3==1 | basura29_1==3 | basura29_2==3 | basura29_3==3)
replace resid_ch=3 if resid_ch ==. & (basura29_1==5 | basura29_2==5 | basura29_3==5 | basura29_1==7 | basura29_2==7 | basura29_3==7)
...
***aguamejorada_ch***
*********************
g       aguamejorada_ch = 1 if (agua15 ==1 & (agua16 ==1 | agua17 ==2)) | agua17 ==1
replace aguamejorada_ch = 0 if (agua15 ==2 & (agua16 ==1 | agua17 ==2)) | agua17 ==2
		
*********************
***banomejorado_ch***
*********************
destring bano*, replace
g       banomejorado_ch = 1 if (bano23 ==1 & (bano24>=1 &  bano24<=3) & (drenaje27>=2 &  drenaje27<=3) & bano26 ==1) | (bano25 ==1 & bano26 ==1) 
replace banomejorado_ch = 0 if (bano23 ==1 & (bano24>=1 &  bano24<=3) & (drenaje27>=2 &  drenaje27<=3) & bano26 ==2) | (bano25 ==1 & bano26 ==2) | ((bano23 ==1) & (drenaje27==1 |  drenaje27==4)) | bano23 ==2 | bano24 ==4
...
**Modificacion Mayra Sáenz - Agosto 2015
gen telef_ch=real(substr(telefono37,1,1))
...
replace vivi1_ch=2 if tipo_viv==4 | tipo_viv==5
replace vivi1_ch=3 if tipo_viv>=6
...
gen vivi2_ch=0
replace vivi2_ch=1 if tipo_viv==1 | tipo_viv==2 |tipo_viv==4 | tipo_viv==5
replace vivi2_ch=0 if tipo_viv>=6
