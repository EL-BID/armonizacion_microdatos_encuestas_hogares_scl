...
*->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<-*
clear all
use "${surveysFolder}\survey\JAM\SLC\1999\m5\data_merge\JAM_1999m5.dta"
set more off
*Main directory
...
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition99") sheetmodify cell(B13)   
restore

 preserve
collapse (max) *res [iw=factor], by(residence)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition99") sheetmodify cell(B14) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition99") sheetmodify cell(B16) 
restore

preserve
collapse (max) *quint [iw=factor], by(quintil)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition99") sheetmodify cell(B24) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition99") sheetmodify cell(B32) firstrow(variables)
restore
*/
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition99") sheetmodify cell(B29) 
restore
