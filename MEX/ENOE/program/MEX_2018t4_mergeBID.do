...
global ruta = "\\Sdssrv03\surveys\\survey\MEX\ENOE\2018\t4\data_orig"
...
saveold "$ruta\COE1T418.dta" , replace

use "$ruta\COE2T418.dta"
destring eda, replace
...
saveold "$ruta\COE2T418.dta", replace

use "$ruta\SDEMT418.dta"
destring eda, replace
drop if r_def!=0 | eda<=11 | eda==99		// Se elimina a aquellos que no sean residentes permanentes, no tengasn cuestionario completo o sean menores de 12 años (sólo se hace cuestionario de ocupación a los residentes mayores de 12 años)		
sort  cd_a  ent  con  v_sel  n_hog h_mud n_ren	
merge cd_a  ent  con  v_sel  n_hog  h_mud  n_ren using "$ruta\COE1T418.dta"
tab _merge
keep if _merge==3
drop _merge
sort  cd_a  ent  con  v_sel  n_hog h_mud n_ren	
merge cd_a  ent  con  v_sel  n_hog  h_mud  n_ren using "$ruta\COE2T418.dta"
tab _merge
drop _merge
foreach i of varlist _all {
local longlabel: var label `i'
