...
clear
set more off 
cd "\\sdssrv03\Surveys\survey\VEN\EHM\2009\s2\data_orig\"

*Identificacion bases de datos
*--------------------------------

use per092.dta, clear
sort entidad control linea serie num_hog
cap drop id_*
cap rename peso pesop
save per092.dta, replace

use hog092.dta, clear
sort entidad control linea serie
cap drop id_* 
cap rename peso pesoh
save hog092.dta, replace

use viv092.dta, clear 
sort entidad control linea serie
cap drop id_*
cap rename peso pesov
save viv092.dta, replace

*Merge
*--------
use hog092.dta, clear
joinby entidad control linea serie using viv092.dta,  _merge(_merge)
tab _merge
drop _merge

joinby entidad control linea serie num_hog using per092.dta, _merge(_merge)
tab _merge
drop _merge
...
saveold "\\Sdssrv03\surveys\survey\VEN\EHM\2009\s2\data_merge\VEN_2009s2.dta", replace
