...
****idh_ch****
**************
tostring id_hogar, generate(hogar_str)
tostring hogar, generate(numhogar_str) 
egen idh_ch = concat(hogar_str numhogar_str)
label variable idh_ch "ID del hogar"
drop hogar_str numhogar_str
...
capture drop lp_ci
gen lp_ci = .
replace lp_ci =        9548 if zona_c==1
replace lp_ci =        7510 if zona_c==0
...
gen lpe_ci = .
replace lpe_ci =         4380 if zona_c==1
replace lpe_ci =         3812 if zona_c==0
...
****************
gen afiliado_ci=.
replace afiliado_ci=1 if asegura>=1 & asegura<=3 
*|(afilpenc>=1 & afilpenc<=6)
...
* 1993 junio (fuente OIT-CEPAL observatorio de la crisis)
* 2015 MGD: se cambia por dato oficial publicado por el M. de Trabajo
gen salmm_ci= 22959.3
label var salmm_ci "Salario minimo legal"
*Para hacer comparable con el resto de las encuestas se acota al grupo de 
*trabajadores mayores a 12 años
...
gen byte eduui_ci=0
replace eduui_ci=1 if (niveduc>=41 & niveduc<=43)
