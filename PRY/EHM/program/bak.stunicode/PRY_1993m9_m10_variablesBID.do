...
use `base_in', clear
/*
la variable ocupacio NO EXISTE 
cod anterior
...
* Cambios: se considera el limite de edad de la encuesta que es 10 anios. 05/27/2014 MGD
gen condocup_ci=.
replace condocup_ci=1 if (trabajo1>=1 & trabajo1<=3)
replace condocup_ci=2 if (trabajo1>=4 & trabajo1<=6 ) & bustrab==1 & edad_ci>=10
replace condocup_ci=3 if (trabajo1>=7 & trabajo1<=12 ) & edad_ci>=10
recode condocup_ci (.=4) if trabajo1==0 & edad<10
...
gen subemp_ci=0
replace subemp_ci=1 if nrohrspn>=1 & nrohrspn<=30 & trabmas==1 & emp_ci==1 
...
gen categopri_ci=.
replace categopri_ci=1 if cateprin ==5
replace categopri_ci=2 if cateprin ==6 
replace categopri_ci=3 if cateprin ==1 | cateprin ==2 | cateprin ==3  | cateprin==4 | cateprin==8
replace categopri_ci=4 if cateprin ==7
replace categopri_ci=. if emp_ci~=1
...
*usando codificacion de 1995 (Variable ocupasal)
...
*** Corrección Marcela Rubio Septiembre 2014: cambio en sintaxis
...
*** secundaria 
replace yedc=7 if ultano==21 | ultano==34
replace yedc=8 if ultano==22 | ultano==35
replace yedc=9 if ultano==23 | ultano==36
replace yedc=10 if ultano==37
replace yedc=11 if ultano==38
replace yedc=12 if ultano==39


*** universitario o superior
replace yedc=13 if ultano==41
replace yedc=14 if ultano==42
replace yedc=15 if ultano==43
replace yedc=16 if ultano==44 
replace yedc=17 if ultano==45 
replace yedc=18 if ultano==46
gen byte aedu_ci=yedc
...
Las categorias cambiaron en el cuestionario este año y los labels de las categorias están mal en la base, el cuestionario indica lo siguiente:
1 Corp. o Senasa
2 Red Privada
3 Arroyo Rio
4 Ycua o Manantial
...
gen agua_c = agua
label define agua_c 1 "Corp. o Senasa" ///
2 "Red Privada" ///
3 "Arroyo Rio" ///
4 "Ycua o Manantial" ///
5 "Pozo o Aljibe" ///
6 "Aguatero" ///
7 "Otro"
label value agua_c agua_c

gen aguared_ch=(agua_c==1 | agua_c==2)
...
***aguamejorada_ch***
*********************
g       aguamejorada_ch = 1 if (agua >=1 & agua <=2) | agua == 5
replace aguamejorada_ch = 0 if (agua >=3 & agua <=4) | (agua >=6 & agua <=7)
