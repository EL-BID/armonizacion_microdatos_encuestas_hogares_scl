...
capture log close
log using "${surveysFolder}\Data.idb\CECILIACA\BANANAS\BELIZE\programs\1999\ingresos_blz99.log", replace
...
set memory 250m
set matsize 800
local out "${surveysFolder}\Data.idb\CECILIACA\BANANAS\BELIZE\CD BELIZE\"
cd "${surveysFolder}\Data.idb\CECILIACA\BANANAS\Belize\Bases\1999\"
...
use "${surveysFolder}\Data.idb\CECILIACA\BANANAS\BELIZE\orig\lfs99enviroment.dta", clear

destring _all, replace
compress

*foreach var of varlist q01 q02 q03 q04 q05 q06 q07 {
*ren `var' `var'_v
*}
...
ren result q07
*sort qid
sort  q01 q02 q03 q04 q05 q06 q07
save "belize99_2", replace

use "belize99.dta", clear
...
merge q01 q02 q03 q04 q05 q06 q07 using "belize99_2.dta"

tab _merge

drop _merge

save "vanblz99.dta", replace


* Identificador del Hogar
egen idh_ch=group(q01 q02 q03 q04 q05 q06 q07)
...
gen subemp_ci=.
replace subemp_ci=1 if horastot_ci<=30 & p36==1
replace subemp_ci=0 if (horastot_ci>30 & horastot_ci<.) | (horastot_ci<=30 & p36==2)
label var subemp_ci "Trabajadores subempleados"

* Trabajadores a Medio Tiempo
gen byte tiempoparc_ci=.
replace tiempoparc_ci=1 if horastot_ci<=30 & p36==2
replace tiempoparc_ci=0 if (horastot_ci>30 & horastot_ci<.) | (horastot_ci<=30 & p36==1)
...
replace tamfirma_ci=1 if p27a==2 | p27a==3 | p27a==4
...
* Sector Publico
gen spublico_ci=.
replace spublico_ci=1 if p26a==3 | p26a==4
replace spublico_ci=0 if p26a==1 | p26a==2 | p26a==5 | p26a==6
...
* OCUPACION (variable 'p28a')
capture drop ocupa_ci 
gen ocupa_ci=.
replace ocupa_ci=1 if p28a>=2000 & p28a<=3999
replace ocupa_ci=2 if p28a>=1000 & p28a<=1999
replace ocupa_ci=3 if p28a>=4000 & p28a<=4999
replace ocupa_ci=4 if p28a>=5200 & p28a<=5999
replace ocupa_ci=5 if p28a>=5000 & p28a<=5199
replace ocupa_ci=6 if p28a>=6000 & p28a<=6999
replace ocupa_ci=7 if p28a>=7000 & p28a<=8999
replace ocupa_ci=8 if p28a>=0 & p28a<=999 
replace ocupa_ci=9 if (p28a>=9000 & p28a<=9996) | p28a==9998
...
* RAMA (variable 'p29a')
gen byte rama_ci=.
replace rama_ci=1 if p29a>=0 & p29a<1000
replace rama_ci=2 if p29a>=1000 & p29a<2000
replace rama_ci=3 if p29a>=2000 & p29a<4000
replace rama_ci=4 if p29a>=4000 & p29a<5000
replace rama_ci=5 if p29a>=5000 & p29a<6000
replace rama_ci=6 if p29a>=6000 & p29a<7000
replace rama_ci=7 if p29a>=7000 & p29a<8000
replace rama_ci=8 if p29a>=8000 & p29a<9000
replace rama_ci=9 if p29a>=9000 & p29a<10000
...
gen antiguedad_ci=p37a if p37a<99
...
gen categopri_ci=.
replace categopri_ci=1 if p26a==1
replace categopri_ci=2 if p26a==2
replace categopri_ci=3 if p26a==3 | p26a==4 | p26a==5
replace categopri_ci=4 if p26a==6
...
TRAB SIN PAGO */
gen categosec_ci=.
replace categosec_ci=1 if p26b==1
replace categosec_ci=2 if p26b==2
replace categosec_ci=3 if p26b==3 | p26b==4 | p26b==5
replace categosec_ci=4 if p26b==6
...
********************************************************************
gen ylm_ci=p38a*(365/12) if p39==1 /* diario */
replace ylm_ci=p38a*(30/7) if p39==2 /* semanal */
replace ylm_ci=p38a*(15/7) if p39==3 /* cada dos semanas */
replace ylm_ci=p38a*2 if p39==4 /* dos veces al mes */
replace ylm_ci=p38a if p39==5 /* mensual */
replace ylm_ci=p38a/(365/30) if p39==6 /* anual */
replace ylm_ci=p38a if p39==7 & categopri_ci==4
...
compress
save "`out'1999\BLZ1999EA_BID.dta", replace
log close
