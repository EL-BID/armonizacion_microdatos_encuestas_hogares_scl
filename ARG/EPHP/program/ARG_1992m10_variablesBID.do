...
gen horastot_ci=p15
replace horastot_ci=. if p15>900
...
label var p14 "horas trabajadas en ocup. pricipar"
label var p471 "ingreso asalariado"
label var p472 "ingreso cuenta propista"
label var p473 "utilidades y beneficios"
label var p474 "Alquileres, rentas, intereses y dividendos"
label var p475 "Jubilacion o pension"
label var p476 "Otros ingresos"
...
gen nodeclap471=((p471~=-9) & (p21~=9 | p21~=0))
gen nodeclap472=((p472~=-9) & (p21~=9 | p21~=0))
gen nodeclap473=((p473~=-9) & (p21~=9 | p21~=0))
gen nodeclap474=(p474~=-9)
gen nodeclap475=(p475~=-9)
gen nodeclap476=(p476~=-9)
...
forvalues m=1(1)6 {
replace p47`m'=0 if nodeclap47`m'==0

gen p47`m'mis=(p47`m'==.)

replace p47`m'=0 if p47`m'mis==1
}
...
gen ylmpri_ci=p21 if ((p471>0 & p471<.) | (p472>0 & p472<.) | (p473>0 & p473<. & categopri_ci==1))
replace ylmpri_ci=p471 if (p21<p471 & p472==0 & p473==0) & p12==1
replace ylmpri_ci=p472 if (p21<p472 & p471==0 & p473==0) & p12==1
replace ylmpri_ci=p473 if (p21<p473 & p471==0 & p472==0) & p12==1 & categopri_ci==1
...
es esto.
Hasta 1994 utilidades y beneficios (p473) era la categoria donde se incorporaban los
ingresos que reciben los patrones, por eso los incorporo aca.
No voy a considerar a p473 como ingreso laboral cuando la persona declara no ser patron o empleador.
************/

gen ylm_ci=p471+p472+p473
replace ylm_ci=ylm_ci-p473 if p473>0 & p473<. & categopri_ci~=1
replace ylm_ci=. if (p471==. & p472==. & p473==.)
...
gen ynlm_ci=p473+p474+p475+p476
replace ynlm_ci=ynlm_ci-p473 if p473>0 & p473<. & categopri_ci==1
replace ynlm_ci=. if (p473==. & p474==. & p475==. & p476==.)
...
/***********
ylmhopri_ci
No se puede crear, ya que no se tienen la cantidad de horas trabajadas en la ocupacion
principal para el periodo 1992-1994
************/
...
forvalues m=1(1)6 {
replace p47`m'=. if nodeclap47`m'==0 | p47`m'mis==1
}
...
/*********************************
Todos las encuestas provienen de la ronda de mayo
**********************************/
...
Variables de Hogar
*******************/
gen vivi1_ch=1 if p01_hogar==1
replace vivi1_ch=2 if p01_hogar==2
replace vivi1_ch=3 if p01_hogar>2 & p01_hogar<=8

gen vivi2_ch=(vivi1_ch==1 | vivi1_ch==2)
replace vivi2_ch=. if vivi1_ch==.

gen cuartos_ch=p03_hogar
replace cuartos_ch=. if p03_hogar==99

gen aguared_ch=.
replace aguared_ch=1 if p04_hogar==1
replace aguared_ch=0 if  p04_hogar==2

gen luz_ch=.
replace luz_ch=1 if p05_hogar==1
replace luz_ch=0 if p05_hogar==2

gen bano_ch=.
replace bano_ch=1 if p06_hogar==1 | p06_hogar==2
replace bano_ch=0 if p06_hogar==3

gen banoex_ch=.
replace banoex_ch=1 if p06_hogar==1
replace banoex_ch=0 if p06_hogar==2
gen des1_ch=.
gen des2_ch=.

gen viviprop_ch=.
replace viviprop_ch=0 if p07_hogar==3
replace viviprop_ch=1 if p07_hogar==1 | p07_hogar==2
replace viviprop_ch=3 if p07_hogar==4 | p07_hogar==5 | p07_hogar==8


gen pared_ch=.
replace pared_ch=0 if p08_hogar==4 | p08_hogar==5
replace pared_ch=1 if p08_hogar<4
replace pared_ch=2 if p08_hogar==8
...
gen vivialqimp_ch=.
    ...
    ***aguamejorada_ch***
    *********************
	gen  aguamejorada_ch = 1 if agua == 1 
	replace aguamejorada_ch = 0 if agua == 2 
	
		
	*********************
    ***banomejorado_ch***
    *********************
	gen banomejorado_ch = 1 if bano == 1
	replace banomejorado_ch = 0 if bano == 2 | bano ==3
...
gen ocupa_ci = .
replace ocupa_ci = 2 if (p20 >=11 & p20 <=99) & emp_ci==1		
replace ocupa_ci = 3 if (p20>=111 & p20<=239)& emp_ci==1
replace ocupa_ci = 4 if (p20>=302 & p20<=394) & emp_ci==1		 									 						
replace ocupa_ci = 5 if (p20>=401 & p20<=594) & emp_ci==1					
replace ocupa_ci = 6 if (p20>=611 & p20<=683) & emp_ci==1 																									
replace ocupa_ci = 7 if (p20>=711 & p20<=997) & emp_ci==1 	
replace ocupa_ci = 8 if (p20>=400 & p20<410) & emp_ci==1 			 																													
replace ocupa_ci = 9 if p20==998 & emp_ci==1  
*/
/************************************************************************************************************
...
*segsoc_ci*** 
*************
gen segsoc_orig=segsoc_ci
drop segsoc_ci
gen segsoc_ci=.
replace segsoc_ci=1 if  p23==2 | p23==3 | p23==6 | p23==7 |p23==10 |p23==11 |p23==14 |p23==15 | p23==18 | p23==19 | p23==22 | p23==23  | p23==26 | p23==27 | p23==30  | p23==31 | p23==34 | p23==35 | p23==38 | p23==39 | p23==42 | p23==43 | p23==46 | p23==47 | p23==50 | p23==51 | p23==54  | p23==55 | p23==58 | p23==59 | p23==62 | p23==63
recode segsoc_ci .=0 if pea3_ci==1 & desemp3_ci==0

*************
*subemp_ci*** 
*************
tab subemp_ci
replace subemp_ci=1 if p15t<35 & p16==1
...
replace rama_ci = 6 if (p18>=500 & p18<=560) & emp_ci==1
replace rama_ci = 7 if (p18>=600 & p18<=642) & emp_ci==1
...
****************
gen cotizando_ci=.
replace cotizando_ci=1 if p23==2 | p23==3 | p23==6 | p23==7 |p23==10 |p23==11 |p23==14 |p23==15 | p23==18 | p23==19 | p23==22 | p23==23  | p23==26 | p23==27 | p23==30  | p23==31 | p23==34 | p23==35 | p23==38 | p23==39 | p23==42 | p23==43 | p23==46 | p23==47 | p23==50 | p23==51 | p23==54  | p23==55 | p23==58 | p23==59 | p23==62 | p23==63
replace cotizando_ci = 0 if p23==1 | p23 == 4 | p23 == 5 | p23 == 8  | p23 == 9 | p23 ==12 | p23 == 13 | p23 == 16 | p23 == 17 | p23 == 20 | p23 == 21 |  p23 == 24 |   p23 == 25 | p23 == 28 | p23 == 29 | p23== 32 | p23 == 33 | p23 == 36 |  p23 == 37 | p23 == 40 | p23 == 41 |  p23 ==  44 | p23 == 45 | p23 == 48 | p23 == 49 | p23 ==  52 | p23 == 53 | p23== 56 | p23 == 57 | p23 == 60 | p23==61
replace cotizando_ci = 0 if p23==64
recode cotizando_ci . = 0 if estado == 1 & p17==  1 | p17 ==2  /*independientes*/
replace cotizando_ci = 0 if estado == 2 
...
gen tipocontrato_ci=.
*este variable no está en el archivo (no fue suministrada por el país)
...
replace tamemp_ci=2 if (p19 >= 3 & p19 <= 5) 
replace tamemp_ci=3 if (p19 >= 6 & p19 < 9)  
...
replace pension_ci = 1 if p475>0 
recode pension_ci .=0 
...
***capital_ci***
***************
egen capital_ci= rowtotal(p473 p474), m 
...
label var ypen_ch "Ingresos del hogar por jubilaciones y pensiones" 
