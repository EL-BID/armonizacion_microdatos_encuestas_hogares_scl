...
replace mes_c= 7  if sema_lev>=1  & sema_lev<=2
replace mes_c= 8  if sema_lev>=3  & sema_lev<=6
replace mes_c= 9  if sema_lev>=7  & sema_lev<=10
replace mes_c= 10 if sema_lev>=11 & sema_lev<=14
replace mes_c= 11 if sema_lev>=15 & sema_lev<=18
replace mes_c= 12 if sema_lev>=19 & sema_lev<=24
*** average week of the survey is 11.17
replace mes_c=10  if mes_c==.
label variable mes_c "Mes de la Encuesta: Segundo Semestre de 1996" 
...
**********
gen zona_c=.
replace zona_c=0 if tipo==2
replace zona_c=1 if tipo==1
...
replace num_per=6 if idh==10506 & num_per==3 & pp12==10
gen idp_ci=num_per
...
/*
gen condocup_ci=.
replace condocup_ci=1 if (codigo_s>=1 & codigo_s <=2) 
replace condocup_ci=2 if codigo_s==3 | codigo_s==8
...
*********
gen lp_ci=.
replace lp_ci=14297.1 if zona_c==1
replace lp_ci=11437.7 if zona_c==0
...
gen ylm_ci=.
replace ylm_ci=pp57 if (pp57>=0 & pp57!=. & pp56d>=0 & pp56d!=.) & (pp56d<=pp57)
replace ylm_ci=pp56d if (pp56d>=0 & pp56d!=. & pp57>0 & pp57!=.) & (pp56d>pp57)
replace ylm_ci=pp57+pp59g if (pp56d>=0 & pp56d!=. & pp57>0 & pp57!=.) & (pp56d<=pp57) & pp59g>=0 & pp59g!=.
replace ylm_ci=pp56d+pp59g if (pp56d>=0 & pp56d!=. & pp57!=.) & (pp56d>pp57) & pp59g>=0 & pp59g!=.
replace ylm_ci=ylm_ci if ylm_ci==. & ylm_ci<.
replace ylm_ci=. if pp12<10
label variable ylm_ci "Ingreso Laboral Monetario Total + Horas Extra"  
replace ylm_ci=ylm_ci/1000
replace ylm_ci=0 if ylm_ci >0 & ylm_ci <1
...
*desde 1999 es ph16b
gen vivialq_ch=.
replace vivialq_ch=ph6b if ph6b>=0 &viviprop_ch==0 
