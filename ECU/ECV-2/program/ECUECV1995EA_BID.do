...
*****  Original by Hermes Martinez, July 13, 2005 ******************************
****************************************************************************

/**** revision August 2,2005 Suzanne and Maria Fernanda

asiste_ci is not conditional on PE7 or PE19
pqnoasiste was also wrong:  not PE19  (PE30)
also, the labels were wrong
...
**********Revision  Enero 30 de 2007 (MFP)
categopri_ci has an error PA20==6 y PA20==12 is for non remunerated employees working outside the household,
Suzanne agreed to change them and categorize them  into categopri==4 NON remunarated workers.
It should be noted that this might be a problem in other surveys and yhat it implies to change our 
concept on non remunerated workers. It should be non remunerated workers including family workers instead 
of just family workers.
...
*/
egen idh_ch=group(region CIUDAD ZONA SECTOR VIVIEN HOGAR NHOGAR numero) 
label variable idh_ch "ID del hogar"
...
*PA30 tiene valores para emp, asalariados y trabajadores domesticos (PA20=1,2, 13) 
*PA22 tiene valores para el resto de los dependientes (PA20=7-trab agro a sueldo-, 8jornalero o peon agrop)
...
/*Con esta forma de calcular el ingreso vamos a perder 4 casos que al parecer estan mal codificados:
1. Un patron que tiene missing en su ingreso correspondiente como patron (PA21) y valor en PA30
2. tres cuenta propias en el mismo caso que el anterior
3. En el caso de los 4 trabajadores agro a sueldo PA20==7 los vamos a recuperar despues
tab  PA20  if PA30!=., nolab
       PA20 |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |        935       19.63       19.63
          2 |      3,418       71.75       91.37
          3 |          1        0.02       91.39
...
la decision fue sacarlo de muestra-reunion con Suzanne*/
...
/*Para los trabajadores independientes
PA21 es para PA3,4,9 y 10 osea categopri 1 y 2*/
...
replace yprid=. if ingrneto>0 & ingrneto<9999999 & categopri_ci<=2
replace yprid=. if yprijbi>0 & yprijbi<9999999 & categopri_ci<=2

egen ylmpri_ci=rsum(yprid ingrneto yprijbi)
replace ylmpri_ci=. if yprid==. & yprijbi==. & ingrneto==.
replace ylmpri_ci=. if emp==0

* Esta variable no se que es ni por que esta creada 
egen ylmpri1_ci=rsum(yprijbi yprijbd)
replace ylmpri1_ci=. if yprijbd==. & yprijbi==.
replace ylmpri1_ci=. if emp==0
...
replace ysec1=PA53*PA48 if emp_ci==1 & PA53A==1 & PA43==1 & PA48<=7
...
***asiste_ci***
***************
*modificado 4 Agosto ver nota al principio del programa*

gen asiste_ci=0
replace asiste_ci=1 if PE13==1 
replace asiste_ci=0 if PE13==1  & PE31>=1 & PE31!=.
replace asiste_ci=. if PE13==.
...
label define pqnoasis_ci 5 "Falta de recursos" 6 "Vacaciones" , add
label define pqnoasis_ci 7 "Trabajo" 8 "No tiene interes" 9 "Mal clima" 10 "otro" , add 
...
gen edupub_ci=.
replace edupub_ci=1 if PE15>=1 & PE15<=6 
