...
* MGR Jul, 2015: modifico sintáxis, existen varios duplicados con llave idh_ch idp_ci
/*
...
tostring componen hogar1 aglomera, replace
capture drop idh_ch
gen str7 idh_ch=codigo+ciudad+componen+hogar1
*Componen tiene muchos missing values
*egen idh_ch= group(codigo ciudad)
*drop codigo ciudad
...
gen str2 ciudad=string(aglomerado)
tostring componen hogar1 aglomera, replace
egen idh_ch=group(codigo ciudad hogar1) 
drop codigo ciudad
...
*MGR Jul, 2015: variable componen tiene varios valores missing, se genera de otra manera
*gen idp_ci=componen
...
*MS Septiembre 2014:
*El 1.89% de observaciones declaran tener 2 o 3 jefes de hogar, por lo que se les clasifica como otros parientes
gen njefes=(relacion_ci==1)
replace njefes=. if relacion_ci==.
sort idh_ch
bys idh_ch: gen njfs=sum(njefes)
tab njfs
replace relacion_ci = 4 if njfs >1 & njfs < 5
drop njfs njefes
...
replace civil_ci=. if h14==9 | h14==6
...
por lo que no se requiere la utilizacion de estas variables.
****************************************************/
capture drop aglomera
g aglomera=aglomerado
destring aglomera, replace
# delimit;
capture destring p20, replace;
gen ocupa = . ;
if (aglomera==3 | aglomera==9 | aglomera==20 | aglomera==26 | aglomera==27 | aglomera==31) {;
local ocup = "ocupa";
local var = "p20";
...
replace ocupa_ci = 9 if ((p20>=998 & p20<=999) | ocup4>=7 | p20==198 | p20==72 | p20==74 | p20==44 | p20==43) & emp_ci==1  & ocupa_ci!=2																											
