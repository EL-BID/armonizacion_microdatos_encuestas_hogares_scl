...
*->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<-*
clear all
use "${surveysFolder}\survey\JAM\SLC\2001\m5\data_merge\JAM_2001m5.dta"
set more off
*Main directory
...
replace e_ano = "2001" if e_ano ~="."


gen emes =  e_mes if e_mes >= "03" & e_mes <= "09"
replace e_mes = emes
...
*replace e_ano = 2000+e_ano
 ...
 gen anios= "19" if  b_year >=90 & b_year <=99
 replace anios = "200" if  b_year ==1 | b_year ==0
...
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition01") sheetmodify cell(B13)   
restore

 preserve
collapse (max) *res [iw=factor], by(residence)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition01") sheetmodify cell(B14) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition01") sheetmodify cell(B16) 
restore

preserve
collapse (max) *quint [iw=factor], by(quintil)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition01") sheetmodify cell(B24) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition01") sheetmodify cell(B32) firstrow(variables)
restore
*/
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition01") sheetmodify cell(B29) 
restore
