...
use "${surveysFolder}\ARM\ECU\ECV\1998\Data\ecu98ecv.dta" 
/*
...
save "${surveysFolder}\ARM\ECU\ECV\1998\Arm_data\ECU1998EA_BID.dta", replace
...
** Gender classification of the population refering to the head of the household.
 ...
 sort CIUDAD  ZONA  SECTOR  VIVIENDA  HOGAR  PD01
...
** Years of education. 
* Population with six or more years of age *

/*
PE29
29. ¿Cuál es o fue el nivel de educación más avanzado que cursa o cursó?
PE30
30. ¿Cuál es o fue el grado, curso, año  más avanzado de educación que cursa o aprobó?

PE29
1. Ninguno
2. Centro de alfabetización
3. Preprimario
4. Educación Básica de Adultos (EBA)
5. Primario
6. Secundario
...
gen ultgrad = PE30
 ...
 replace anoest=0  if (ultniv>=1 & ultniv<=4) | (ultniv==5 & ultgrad==0) 
 replace anoest=1  if (ultniv==5 & ultgrad==1) 
 replace anoest=2  if (ultniv==5 & ultgrad==2) 
 replace anoest=3  if (ultniv==5 & ultgrad==3) 
 replace anoest=4  if (ultniv==5 & ultgrad==4) 
 replace anoest=5  if (ultniv==5 & ultgrad==5) 
 replace anoest=6  if (ultniv==5 & ultgrad==6) | (ultniv==6 & ultgrad==0) 
 replace anoest=7  if (ultniv==6 & ultgrad==1) 
 replace anoest=8  if (ultniv==6 & ultgrad==2)
 replace anoest=9  if (ultniv==6 & ultgrad==3)
 replace anoest=10 if (ultniv==6 & ultgrad==4)
 ...
 replace anoest=16 if (ultniv==7 & ultgrad==4 ) | (ultniv==8 & (ultgrad>=4 & ultgrad<=6))
 replace anoest=17 if (ultniv==7 & ultgrad==5)
 replace anoest=18 if (ultniv==7 & (ultgrad>=6 & ultgrad<=8)) |(ultniv==9 & ultgrad==0)
...
* 10 years or more
 ...
 replace peaa=2 if (PA04>=1 & PA04<=6)
 ...
 label define ramarlabel 1 "agricultura, ganaderia y pesca" 2 "resto"
...
/*
pe10 idioma que habla(6 años y más) 
 1. EsPAñol
 2. Quichua
 3. Shuar
 4. EsPAñol y Quichua
...
*/
 ...
 gen	 indigena=1 if (PE10>=2 & PE10<=6)
 replace indigena=0 if (PE10==1 | PE10==7)
...
/*
pe11
11.Se matriculó en el presente año escolar en:

1. Primaria
2. Educ. Basica. Adultos
3. Secundaria
4. Universidad
5. Sup. no univ
6. Postgrado
...
pe14
En que grado, curso o año se matriculó ... en el presente
año escolar

pe14a Incluyendo esta, ¿cuántas veces se ha matriculado en este
grado, curso o año?
*/

 gen	 NERP=0 if (edad>=6 & edad<=11) & (nivmatric>=1 & nivmatric<=7)
 replace NERP=1 if (edad>=6 & edad<=11) & (nivmatric==1)

** Target 3, Additional Indicator: Net Attendance Ratio in Secondary
* ISCED 2 & 3

 gen	 NERS=0 if (edad>=12 & edad<=17) & (nivmatric>=1 & nivmatric<=7)
 replace NERS=1 if (edad>=12 & edad<=17) & (nivmatric==3)

** Upper secondary
* 2o Ciclo de enseñanza secundaria

 gen     NERS2=0 if (edad>=15 & edad<=17) & (nivmatric>=1 & nivmatric<=7)
 replace NERS2=1 if (edad>=15 & edad<=17) & (nivmatric==3) & (PE14>=3 & PE14<=6)
...
/*
*PE31 ha obtenido algun documento o certificado por los años de
      estudio que ha realizado	
         1. Si => 32
         2. No => 33
         
*PE32 último documento que obtuvo
1 y 2 PAse a 33
3 a 8 PAse a 34

*PE33. ¿Sabe leer y escribir? (PErsonas de 6 años y más)
 1. Sólo leer
 2. Leer y escribir
 3. Ninguno
*/

 gen	 alfabet=1 if ((PE31==1 & (PE32>=3 & PE32<=8)) |  PE33==2)
 replace alfabet=0 if (PE33==1 | PE33==3)

 gen 	 ALFABET2=0 if (edad>=15 & edad<=24) & (alfabet>=0 & alfabet<=1)
 replace ALFABET2=1 if (edad>=15 & edad<=24) & (alfabet==1) 

** GOAL 3 PROMOTE GENDER EQUALITY AND EMPOWER WOMEN

 gen prim=1 if (nivmatric==1)
 gen sec=1  if (nivmatric==3)
 gen ter=1  if (nivmatric==4 | nivmatric==5)
...
3. PAtrono o socio
4. Cuenta propia
5. Trabajador del hogar sin PAgo
6. Trabajador no del hogar sin PAgo

** AGROPECUARIOS

7. Trab. Agrop. a Sueldo y Salario
8. Jornalero/PEón AgroPEcuario
9. PAtrono o socio de la finca
10. Trab. Agrop. por cuenta propia	
11. Ayudante agroPEc. del hogar sin PAgo
12. Ayudante agroPEc. no del hogar sin PAgo

13. Empleado/a doméstico/a		

*/

 gen	 WENAS=0 if (edad>=15 & edad<=64) & (categ>=1 & categ<=2) & (peaa==1)
 replace WENAS=1 if (edad>=15 & edad<=64) & (categ>=1 & categ<=2) & (peaa==1) & (sexo==2)
	
 gen	 WENASD=0 if (edad>=15 & edad<=64) & ((categ>=1 & categ<=2) | categ==13) & (peaa==1)
 replace WENASD=1 if (edad>=15 & edad<=64) & ((categ>=1 & categ<=2) | categ==13) & (peaa==1) & (sexo==2)
...
*/
 ...
 gen	 SKILLED=0 if (qaten>=1 & qaten<=7) & (edad>=15 & edad<=49)
 replace SKILLED=1 if (qaten>=1 & qaten<=4) & (edad>=15 & edad<=49)
...
pf21. ¿Utiliza usted algún método PAra no quedar embarazada?

pf22. ¿Qué método utiliza?
1. Ritmo
2. Retiro
3. Píldora
4. DIU
...
*/
 ...
 gen	 CONTRACEPTIVE=0 if (sexo==2) & (edad>=15 & edad<=49) & (estcivil==1 | estcivil==2)
 replace CONTRACEPTIVE=1 if (sexo==2) & (edad>=15 & edad<=49) & (estcivil==1 | estcivil==2) & (PF21==1)

/*Condom Use of the Contraceptive Prevalence Rate*/

 gen	 CONDOM=0 if (edad>=15 & edad<=49) & (estcivil==1 | estcivil==2) & (PF21==1) & (sexo==2)
 replace CONDOM=1 if (edad>=15 & edad<=49) & (estcivil==1 | estcivil==2) & (PF21==1) & (sexo==2) & (PF22==5)
...
/*
vi26
26. ¿Con que tipo de alumbrado cuenta principalmente este hogar?
...
* Gender classification of the population refers to the head of the household.
 ...
 gen	 ELEC=0 if (vi26<=4) /* Total population excluding missing information */
 replace ELEC=1 if (vi26<=2)
 	
** Target 9, Indicator: Proportion of the population using solidfuels (%)
/*

vi09
¿En qué sitio de la vivienda cocina los alimentos este hogar?
1. En un cuarto dedicado sólo para cocinar
...
vi11 
¿Con qué se cocina principalmente en este hogar?
...
* Gender classification of the population refers to the head of the household.
 ...
 gen	 SFUELS=0 if ((combusti<=5) | vi09==5) /* Total population excluding missing information */
 replace SFUELS=1 if (combusti==2)

** Target 10, Indicator: Proportion of the population with sustainable access to an improved water source (%)

/*
18. ¿De dónde obtiene el agua  este hogar?	20. Donde esta ubicado el suministro 	21. El suministro queda
						del agua				vi21
											1. Muy cerca
* vi18. fuente de agua				*vi20					2. Cerca
											3. Lejos
1. red pública			==>19		1. Dentro de la vivienda		4. Muy lejos
2. red y carro repartidor   	==>19		2. Fuera de la vivienda, pero dentro del terreno								
3. pila o llave pública		==>19		3. Fuera de la vivienda, lote o terreno
4. otra fuente por tubería 	==>20			
5. carro repartidor/triciclo 	==>20
6. pozo		 	      	==>20		
7. río, vertiente o similar	==>20							
8. agua de lluvia		==>20	
9. otro 			==>20	

vi19
19. ¿El abastecimiento de agua es?
1. Permanente 	==>20
2. Irregular	==>20
*/
...
* Gender classification of the population refers to the head of the household.
 ...
 gen	 WATER=0 if (agua>=1 & agua<=9) /* Total population excluding missing information */
 replace WATER=1 if ((agua>=1 & agua<=4) | agua==6 |agua==8) 

** Target 10, Indicator: Proportion of Population with Access to Improved Sanitation, Urban and Rural (%)

/*								
* vi13. servicio higienico					vi14. 
¿Con qué tipo de servicio higiénico cuenta el hogar?		¿Cuántos cuartos de servicio higiénico
								utilizan las personas de este hogar?
1. excusado y alcantarillado		=>14			vi14	1. En total
2. excusado y pozo séptico		=>14			vi14a 	2. Exclusivos
3. excusado y pozo ciego		=>14			vi14b 	3. Compartidos
4. letrina				=>14
5. no tiene	==>16
...
* vi35. Tenencia de la vivienda			vi05
La vivienda que ocupa este hogar es:		Tipo de vivienda	

1. En arriendo					1. Casa, villa 
2. propia y la está pagando			2. Departamento
3. propia y totalmente pagada			3. Cuarto en inquilinato
4. cedida					4. Mediagua
5. recibida por servicios			5. Rancho/choza/covacha
6. otra						6. Otro 
...
1. duela/parket					1. Hormigón/bloque/ladrillo
2. baldosa/vinyl				2. Adobe o tapia
3. cemento/ ladrillo				3. Madera
4. Tabla/tablón					4. Bahereque (caña revestida)
5. caña						5. Caña
6. tierra					6. Otro
7. otro  	

* vi06 Número de cuartos 
...
* 1. Non secure tenure or type of dwelling.
 ...
 gen secten_1=0     if (tipoviv>=1 & tipoviv<=6) & (tenencia>=1 & tenencia<=6) /* Total population excluding missing information */
 replace secten_1=1 if (tipoviv==5) | (tenencia>=4 & tenencia<=5)
...
* 30. ¿Tiene este hogar servicio teléfonico?
