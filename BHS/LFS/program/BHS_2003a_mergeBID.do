...
global ruta = "\\Sdssrv03\surveys\\survey\\BHS\LFS\\2003\\a\\data_orig"
...
use  "$ruta\Bahamas_LFS_2003_housing.dta", clear
capture drop sup_dist2
gen sup_dist2  = string(sup_dist,"%03.0f")
capture drop enum_dist2
gen enum_dist2  = string(enum_dist,"%02.0f")
capture drop id
egen id=concat(island sup_dist2 enum_dist2 hhno)
sort id
save "$ruta\Bahamas_LFS_2003_housing.dta", replace

use "$ruta\Bahamas_LFS_2003_individual.dta", clear
gen sup_dist2  = string(sup_dist,"%03.0f")
gen enum_dist2  = string(enum_dist,"%02.0f")
egen id=concat(island sup_dist2 enum_dist2 hhno)
sort id
merge id using "$ruta\Bahamas_LFS_2003_housing.dta"
tab _merge
keep if _merge==3
drop _merge
