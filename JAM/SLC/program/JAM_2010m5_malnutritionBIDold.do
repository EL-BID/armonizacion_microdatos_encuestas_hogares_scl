...
*->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<->:<-*
clear all
use "Y:\survey\JAM\SLC\2010\m5\data_merge\JAM_2010m5.dta"
set more off
*Main directory
...
replace e_ano = 2010 if e_ano ==10

gen emes =  e_mes if e_mes >= 2 & e_mes <= 11
replace e_mes = emes
...
*replace e_ano = 2000+e_ano
 ...
 replace anios = 200 if  (b_year >=0 & b_year <=10)
...
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition10") sheetmodify cell(B13)   
restore

 preserve
collapse (max) *res [iw=factor], by(residence)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition10") sheetmodify cell(B14) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition10") sheetmodify cell(B16) 
restore

preserve
collapse (max) *quint [iw=factor], by(quintil)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition10") sheetmodify cell(B24) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition10") sheetmodify cell(B32) firstrow(variables)
restore
*/
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition10") sheetmodify cell(B29) 
restore
