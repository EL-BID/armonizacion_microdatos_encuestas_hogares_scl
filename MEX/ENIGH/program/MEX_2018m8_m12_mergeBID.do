...
global ruta = "\\Sdssrv03\surveys\\survey\MEX\ENIGH\2018\m8_m12\data_orig"
...
Última versión: Alvaro Altamirano, E-mail: alvaroalt@iadb.org
...
replace ing_1=(ing_1/dic18)/12 if clave=="P009" | clave=="P016"
...
saveold "$ruta\ingreso_deflactado18_per.dta", replace
...
save "$ruta\ingresonomonetario_def18.dta", replace

use "$ruta\ingresonomonetario_def18.dta", clear
...
save "$ruta\esp_def18.dta", replace
...
save "$ruta\reg_def18.dta", replace
...
merge folioviv foliohog using "$ruta\ingreso_deflactado18_per.dta"
tab _merge
drop _merge
*/
...
merge 1:1 folioviv foliohog using "$ruta\esp_def18.dta"
tab _merge
drop _merge
...
merge 1:1 folioviv foliohog using "$ruta\reg_def18.dta"
tab _merge
drop _merge
...
save "$ruta\ingresotot18.dta", replace

saveold "$ruta\gtos_autoc18.dta", replace
...
merge 1:1 folio numren using "$ruta\ingreso_deflactado18_per.dta", keep (match master)
...
merge m:1 folioviv foliohog using "$ruta\gtos_autoc18.dta", keep (match master)
drop _merge
