...
foreach file in annual edtotals exp96 food gifts heads hhsize homegift housing laborf meals noncons nutr rec001 rec002 rec003 rec004 rec005 rec006 rec007 rec008 rec009 rec010 rec011 rec012 rec013 rec014 rec015 rec016 rec017 rec018 rec019 rec020 rec021 rec022 rec023 rec024 rec025 rec026 rec027 rec028 rec029 rec030 rec031 rec032 rec032a rec033 rec034 {
	usespss using "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\spss\\`file'.sav"
	foreach v of varlist _all {
	local lowname=lower("`v'")
	rename `v' `lowname'
}
saveold "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\\`file'.dta", replace
clear all
set more off
}
...
foreach t in annual exp96 heads hhsize housing rec001 rec022 rec029 rec030 rec032a rec033{
	set more off
	use "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\\`t'.dta"
	di "`t'"
...
clear all
use "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\annual.dta"
foreach t in exp96 heads hhsize housing rec001 rec022 rec029 rec030 rec032a rec033 {
	merge m:m serial  using "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\\`t'.dta", force
	di "`t'"
	tab _merge
	drop _merge
	}
sort serial
saveold "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\hogares.dta", replace
...
foreach t in nutr  rec012 rec013 rec014 rec015 rec016 rec017 rec018 rec019 rec020 rec025  rec028 rec034{
	...
	codebook id
	saveold "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\\`t'id.dta", replace
		}


*Esta base tiene caracteres no num'ericos
 use "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\nutrid.dta", clear
 gen id1= id if id ~=".0."
...
destring id, replace
drop if serial ==.
 saveold "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\nutrid.dta", replace



*Merge personas
clear all
use "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\rec034id.dta"
...
foreach t in nutrid rec012id rec013id rec014id rec015id rec016id rec017id rec018id rec019id rec020id rec025id  rec028id  {
	merge m:m id using "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\\`t'.dta", force
	di "`t'"
	drop if id ==.
	drop if _merge ==2
	tab _merge
...
sort serial
saveold "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\personas.dta", replace

merge m:m serial using  "${surveysFolder}\survey\JAM\SLC\1996\m5\data_orig\stata\hogares.dta"
drop if _merge ==2
tab _merge
drop _merge
saveold "${surveysFolder}\survey\JAM\SLC\1996\m5\data_merge\JAM_1996m5.dta", replace
