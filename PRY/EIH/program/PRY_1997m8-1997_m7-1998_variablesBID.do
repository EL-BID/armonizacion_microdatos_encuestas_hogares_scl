...
*HORAS
se agrego la siguiente condicion a la creacion de las horas 
replace hr_seman=. if hr_seman==0
*horas=0 significa NO APLICA*
...
/*dejo 98 pero la encuesta empieza desde 1997*/
...
label define mes_c 1 "Enero-98" 2 "Febrero-98" 3 "Marzo-98" 4 "Abril-98 " 5 "Mayo-98"
label define mes_c 6 "Junio-98" 7 " Julio-98" 8 "Agosto-97",add
label define mes_c 9 "Septiembre-97" 10 "Octubre-97" 11 "Noviembre-97" 12 "Diciembre-97", add
...
gen lp_ci =.
replace lp_ci= 185968  if dominio==1 | dominio==2   /*area metropolitana*/
replace lp_ci= 111124 if dominio==3 | dominio==5   /*rural*/
replace lp_ci= 130940 if dominio==4  /*resto urbano*/
...
gen lpe_ci =.
replace lpe_ci= 94648  if dominio==1 | dominio==2   /*area metropolitana*/
replace lpe_ci= 67211 if dominio==3 | dominio==5   /*rural*/
replace lpe_ci= 72634 if dominio==4  /*resto urbano*/
...
replace condocup_ci=2 if (trabasal==6 | trainoag==6 | trabag==6) & (busco7==1 )
...
gen condocup_ci=.
replace condocup_ci=1 if (peades==1 | peades==4 | peades==5) & edad_ci>=10
replace condocup_ci=2 if (peades==2 | peades==7) & edad_ci>=10
recode condocup_ci .=3 if (peades==3 | peades==6) | (condocup_ci==. & edad_ci>=10)
recode condocup_ci (.=4) if edad_ci<10 | peades==0
...
replace tothoras=. if tothoras==0
...
replace firmapeq_ci=0 if tamestp>5 & tamestp<9999
replace firmapeq_ci=1 if tamestp >=1 & tamestp<=5
*/
*****************
***spublico_ci***
*****************
...
gen ocupa_ci=.
replace ocupa_ci=1 if ocuprin==1  & emp_ci==1
replace ocupa_ci=2 if ocuprin==2  & emp_ci==1
replace ocupa_ci=3 if ocuprin==3  & emp_ci==1
replace ocupa_ci=4 if ocuprin==4  & emp_ci==1
replace ocupa_ci=5 if ocuprin==9  & emp_ci==1
replace ocupa_ci=6 if ocuprin==5  & emp_ci==1
replace ocupa_ci=7 if (ocuprin>=6 & ocuprin<=8)  & emp_ci==1
replace ocupa_ci=9 if ocuprin==10  & emp_ci==1
...
*Modificación MLO - 2015/12
con este supuesto salen mal el indicador de personas asalariadas en empresas pequeñas - por eso se elimina
*/
...
egen ynlm_ci=rsum(yalqrent1 yinteres1 yayudafa1 yotrosy1 yjubpen1 yayudex1), missing
...
gen rentaimp_ch=alqimdo
replace rentaimp_ch=. if alqimdo>=9999999
...
replace yedc=6 if nivgrado==26 | nivgrado==29
...
replace yedc=14 if nivgrado==72 | nivgrado==82
replace yedc=15 if nivgrado==73 | nivgrado==83
replace yedc=16 if nivgrado==74 | nivgrado==84
...
gen asiste_ci=.
replace asiste_ci=1 if asiste>=1 & asiste<=5 
...
gen vivialqimp_ch=alqimdo
replace vivialqimp_ch=. if alqimdo>=9999999
