...
* aedu_ci      * 
**************** 
* Mod.8/2015 Ivan Bonacelli EDU/SCL  (2 lineas)
...
gen aedu_ci=.
* Mod.8/2015 Ivan Bonacelli EDU/SCL  (2 lineas)
replace aedu_ci=.  if e6a==4 // Educación Especial- se excluye
replace aedu_ci=0              if e6a>=1 & e6a<=3    /*Pre-escolar, o ninguna*/
*replace aedu_ci=0              if e6a>=1 & e6a<=4    /*Pre-escolar, especial o ninguna*/
replace aedu_ci=min(e6c,6)     if e6a==5             /*Preparatoria  (Sist. antiguo)*/
replace aedu_ci=min(e6c,8)     if e6a==6             /*Básica (Sist. nuevo) */
replace aedu_ci=min(e6c+6,12)  if e6a==7             /*Humanidades (Sist. antiguo)*/
replace aedu_ci=min(e6c+8,12)  if e6a==8             /*Educación Media Científico Humanística (Sist. nuevo)*/
replace aedu_ci=min(e6c+6,12)  if e6a==9             /*Técnica, Comercial, Industrial o Normalista (Sist. antiguo)*/
replace aedu_ci=min(e6c+8,12)  if e6a==10            /*Educación Media Técnica Profesional (Sist. nuevo)*/  
replace aedu_ci=min(e6c+12,17) if e6a>=11 & e6a<=12  /*Superior */
replace aedu_ci=e6c+12         if e6a==13	        /*Posgrado*/
label var aedu_ci "Anios de educacion aprobados" 
*Nota: a diferencia del 2009 aki no se debe restar un anio ya que pregunta directamente los anios aprobados
...
recode dis_ci nonmiss=. if s37t1>=. & s37t2>=. & s37t3>=.
recode dis_ci nonmiss=. if inlist(9,s37t1, s37t2, s37t3)
forvalues i=1/3 {
forvalues j=1/6 {
replace dis_ci=1 if s37t`i'==`j'
}
}
