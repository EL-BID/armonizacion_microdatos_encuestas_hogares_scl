...
gen lp_ci =.
replace lp_ci= 211175  if dominio==1 | dominio==2   /*area metropolitana*/
replace lp_ci= 125457 if dominio==3 | dominio==5   /*rural*/
replace lp_ci= 148278 if dominio==4  /*resto urbano*/
...
gen lpe_ci =.
replace lpe_ci= 104293  if dominio==1 | dominio==2   /*area metropolitana*/
replace lpe_ci= 74060 if dominio==3 | dominio==5   /*rural*/
replace lpe_ci= 80036  if dominio==4  /*resto urbano*/
...
/*
gen condocup_ci=.
replace condocup_ci=1 if peagru==1
replace condocup_ci=2 if peagru==2
replace condocup_ci=3 if peagru==3 & edad_ci>=10
...
* l condicionar por busqueda y disponibilidad el desempleo baja y no coincide con cifras externas como el SEDLAC.
gen condocup_ci=.
replace condocup_ci=1 if (peades==1 | peades==4 | peades==5)
replace condocup_ci=2 if (peades==2 | peades==7) 
...
**************
/*
recode ocupr (0=.) (99=.) (5=6) (6/8=7)(9=5)(10=9), gen (ocupa_ci)
...
gen ocupa_ci=.
replace ocupa_ci=1 if (ocuprin>=680 & ocuprin<=835)  & emp_ci==1
replace ocupa_ci=2 if (ocuprin>=1 & ocuprin<=676) & emp_ci==1
...
replace ocupa_ci=9 if (ocuprin>=2306 & ocuprin<=2352) & emp_ci==1
...
replace yocasio1=. if yocasio==0 | yocasio==9999999 /*No aplicable*/
...
replace rentaimp_ch=. if alqimdo>=99999999
...
gen aguamala_ch=(agua==4 | agua==5 | agua==8 | agua==9)
...
gen telef_ch=.
replace telef_ch=0 if telefono<6
replace telef_ch=1 if telefono==2 | telefono==3
...
gen cel_ch=.
replace cel_ch=0 if telefono<6
replace cel_ch=1 if telefono==1 | telefono==3
...
replace vivialqimp_ch=. if alqimdo>=99999999
...
bys idh_ch: gen aux=raza_idioma_ci if parentco==1
bys idh_ch: egen aux1 = max(aux)
