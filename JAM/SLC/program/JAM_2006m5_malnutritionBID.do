...
*replace e_ano = 2000+e_ano
 ...
 replace anios = "200" if  b_year ==0 | b_year ==1 | b_year ==2 | b_year ==3 | b_year ==4 | b_year ==5 | b_year ==6
...
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition06") sheetmodify cell(B13)   
restore

 preserve
collapse (max) *res [iw=factor], by(residence)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition06") sheetmodify cell(B14) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition06") sheetmodify cell(B16) 
restore

preserve
collapse (max) *quint [iw=factor], by(quintil)
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition06") sheetmodify cell(B24) 
restore
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition06") sheetmodify cell(B32) firstrow(variables)
restore
*/
...
saveold "$outputs\malnut_temporal.dta", replace
export excel using "$outputs\health_indicators.xlsx", sheet("Malnutrition06") sheetmodify cell(B29) 
restore
